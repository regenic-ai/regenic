...
做一个**本地优先**、服务单个使用者的加工闭环。拉取与推送都算正式接入；
Regenic 只加工信息，不生产渠道内容。

- [ ] 本地权威库（SQLite 或单机 Postgres）+ 本地 Blob
- [ ] 至少一个真实渠道的 ChannelConnector（拉取和/或推送）
- [ ] 过滤与分层，落入 Event / Blob（形状对齐 RFC 0005，范围限个人）
- [ ] 在管道上提供加工结果入口：优先级、「该知道」、跟进（如未回复）等——
      它们是输出，不是产品定义
- [ ] 开放导出（Markdown / JSONL）
- [ ] 可选远端历史（默认关，由用户控制；不是组织库）
- [ ] 个人规则 / 轻量标准钩子（通向 RFC 0001）

**完成标准：** 一个人能在本地接入真实渠道，走完 过滤 → 分层 → 提炼 → 行动，
不依赖厂商云，并能导出自己的数据。
