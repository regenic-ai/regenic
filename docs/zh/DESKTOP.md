...
`/v1/me` 只在数值回环监听时开放（`LISTEN_HOST` 为 `127.0.0.1` 或 `::1`）。`localhost` 仍可作为经过 key 认证的浏览器 Origin，但不能作为 Personal API 监听地址，避免名称解析到非回环接口。Sealos 等 `0.0.0.0` 部署没有这条个人面；`REGENIC_PERSONAL_API=1` 也不能绕过该边界。经过认证的远程身份协议实现前，桌面不能读取远程 Personal API。`REGENIC_PERSONAL_API=0` 可在本机也关掉。桌面 sidecar 会丢掉父进程里的 `REGENIC_PERSONAL_API`，以免外壳 `=0` 把内核个人面关掉。个人模式 CORS 只回显 `file://`、`null` 和本机 Origin；所有带 Origin 的 `/v1/me` 请求还必须携带 sidecar 的临时 key。安装 DSH 时不接收 `command` / `workdir`。本机表单里的 `base_url` 必须是回环 URL。托管 API（Sealos 等）用环境变量 `REGENIC_DSH_BASE_URL` 指向集群内网 dsh web（例如 `http://regenic-dsh:3080`），目录不再要公网地址；已存的 CLI 安装也会按 web 认会话、允许回复。
