...
@Injectable()
export class DailyDigestService implements OnModuleDestroy {
  ...
  async runOnce(now = new Date()): Promise<void> {
    ...
    try {
      ...
      const policy =
        (await (host.get("daily-digest-policy") as DailyDigestPolicyStore).getDailyDigestPolicy(
          this.runtime.orgId(),
        )) ?? DEFAULT_DAILY_DIGEST_POLICY;
      ...
      for (const job of claimed) {
        try {
          ...
          await jobs.completeDailyDigestJob({
            id: job.id,
            owner: this.owner,
            completed_at: at,
          });
        } catch (error) {
          if (error instanceof DailyDigestLeaseLostError) {
            continue;
          }
...
async function withDailyDigestLease<T>(
  ...
  try {
    ...
    await heartbeat;
    if (leaseLost) {
      throw new DailyDigestLeaseLostError();
    }
    return value;
  } finally {
    clearInterval(timer);
