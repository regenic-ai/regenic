...
export function mergeHeadPages(
  ...
  const recentIds = new Set(recent.map((item) => item.thread_id));
  const byThread = new Map<string, InboxViewItem>();
  ...
  const liveIds = recencyLiveThreadIds([...byThread.values()], pageSize);
  const older = previous.filter((item) => {
    if (recentIds.has(item.thread_id) || item.pinned) {
      return false;
    }
    return !liveIds.has(item.thread_id);
  });
  const reusedRecent = reuseInboxItems(
    previous.filter((item) => recentIds.has(item.thread_id)),
    recent,
  );
  return older.length === 0 ? reusedRecent : [...older, ...reusedRecent];
}

function recencyLiveThreadIds(
  items: InboxViewItem[],
  pageSize: number,
): Set<string> {
  const ranked = items
  ...
  return new Set(
    ranked.slice(0, Math.max(pageSize, 0)).map((entry) => entry.item.thread_id),
  );
}
...
function compareHeadRecencyDesc(
  ...
  if (isBeforeEvent(left.event, right.event.occurred_at, right.event.id)) {
    return 1;
  }
  if (isBeforeEvent(right.event, left.event.occurred_at, left.event.id)) {
    return -1;
  }
  return 0;
}
