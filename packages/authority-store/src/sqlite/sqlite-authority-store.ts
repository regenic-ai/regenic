    ...
    ContextProjectionOutboxStore
{
  ...
  async renewContextProjectionJob(input: RenewContextProjectionJob): Promise<boolean> {
    ...
    return this.database.prepare(
      `UPDATE context_projection_outbox SET lease_expires_at = ?, updated_at = ? WHERE id = ? AND status = 'running' AND lease_owner = ? AND lease_expires_at > ?`,
    ).run(expiresAt, input.now, input.id, input.owner, input.now).changes === 1;
  }
